...
Meant to be used in 1:1 client-server connection scenarios, for example on a smartnic (Bluefield) leasing a single address to the host.
### Configuration
IP address to lease shall be passed as a string.
### Notes
...
Delivers the HTTP boot image as a [BootFileURL](https://www.rfc-editor.org/rfc/rfc5970.html#section-3.2). Based on configuration it delivers either client-specific UKIs dynamically or a default UKI for all clients. When client-specific UKIs are configured, IPv6 relays *must* be used, so the client can be identified based on its link-local address (which the relay always provides).

### Configuration
Single HTTP(s) URL shall be passed as a string. It must be either
...
- IP addresses are just created/updated, they are not deleted upon DHCP IP address release. Cleanup process till tbd.
...
## OOB
The OOB plugin leases an IP object to an out-of-band client, based on a subnet detection.
